...
<form action="<?= base_url('admin/berita/update/' . $berita['id']) ?>" method="POST" enctype="multipart/form-data" id="form-berita" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <?= csrf_field() ?>
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <input type="text" name="judul" value="<?= old('judul', $berita['judul']) ?>" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:border-[#00A859] focus:ring-2 focus:ring-[#00A859]/20 text-lg font-medium">
            ...
            <div class="mt-5">
                ...
                <input type="hidden" name="konten" id="konten" value="<?= htmlspecialchars(old('konten', $berita['konten']) ?? '') ?>">
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div id="waktu-tayang-container" class="<?= old('status', $berita['status']) == 'draft' ? 'hidden' : '' ?> mt-4 p-4 rounded-xl border <?= old('status', $berita['status']) == 'terjadwal' ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200' ?>">
                <div class="flex items-center justify-between mb-2">
                    <label id="label-waktu" class="text-sm font-bold <?= old('status', $berita['status']) == 'terjadwal' ? 'text-blue-800' : 'text-gray-700' ?>">
                        <?= old('status', $berita['status']) == 'terjadwal' ? 'Jadwal Tayang <span class="text-red-500">*</span>' : 'Ubah Tanggal Rilis' ?>
                    </label>
                    <button type="button" id="btn-reset-waktu" class="<?= old('status', $berita['status']) == 'terbit' ? 'block' : 'hidden' ?> text-xs text-red-500 font-bold">Kosongkan</button>
                </div>
                <input type="datetime-local" id="waktu_tayang" name="waktu_tayang" value="<?= old('waktu_tayang', $berita['waktu_tayang'] ? date('Y-m-d\TH:i', strtotime($berita['waktu_tayang'])) : '') ?>" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 <?= old('status', $berita['status']) == 'terjadwal' ? 'focus:border-blue-500 focus:ring-blue-500' : 'focus:border-[#00A859] focus:ring-[#00A859]/20' ?>">
                <p id="desc-waktu" class="text-xs mt-2 <?= old('status', $berita['status']) == 'terjadwal' ? 'text-blue-600' : 'text-gray-500' ?>">
                    <?= old('status', $berita['status']) == 'terjadwal' ? 'Tidak terlihat sebelum jadwal.' : 'Kosongkan jika tidak ingin mengubah.' ?>
                </p>
            </div>
            ...
            <div class="mt-5">
                ...
                <select name="id_kategori" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#00A859]">
                    ...
                    <?php foreach ($kategori as $k) : ?>
                        <option value="<?= $k['id'] ?>" <?= old('id_kategori', $berita['id_kategori']) == $k['id'] ? 'selected' : '' ?>><?= esc($k['nama_kategori']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mt-5">
                ...
                <select id="tags" name="tags[]" multiple placeholder="Pilih atau ketik tag..." autocomplete="off">
                    <?php $oldTags = old('tags', $selectedTags ?? []); foreach ($tags as $t) : ?>
                        <option value="<?= $t['id'] ?>" <?= in_array($t['id'], $oldTags) ? 'selected' : '' ?>><?= esc($t['nama_tag']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mt-5">
                ...
                <select name="layout" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl">
                    <option value="modern" <?= old('layout', $berita['layout']) == 'modern' ? 'selected' : '' ?>>Modern (Gambar Lebar)</option>
                    <option value="legacy" <?= old('layout', $berita['layout']) == 'legacy' ? 'selected' : '' ?>>Legacy (Gambar Samping)</option>
                </select>
            </div>
            ...
            <div class="mt-5">
                ...
                <?php if (!empty($berita['gambar'])) : ?>
                    <div class="mb-3">
                        <img src="<?= base_url('uploads/berita/' . $berita['gambar']) ?>" class="w-full h-40 object-cover rounded-xl border" alt="Thumb Lama">
                        <p class="text-xs text-gray-400 mt-1 italic">Gambar saat ini. Biarkan kosong jika tidak ganti.</p>
                    </div>
                <?php endif; ?>
                <input type="file" id="gambar" name="gambar" accept="image/jpeg,image/png,image/webp" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-[#00A859] file:text-white hover:file:bg-[#0B4A2D] border border-gray-200 rounded-xl p-2 bg-gray-50">
                ...
                <p class="text-xs text-gray-500 mt-2">Otomatis WebP 1200px, max 6MB.</p>
            </div>

            <button type="submit" class="w-full mt-6 bg-[#00A859] hover:bg-[#0B4A2D] text-white font-bold py-3 rounded-xl shadow">Simpan Perubahan</button>
        </div>
    </div>
</form>
...
<script>
    ...
    function updateWaktuUI() {
        ...
        if (s === 'terjadwal') {
            ...
            labelWaktu.textContent = 'Ubah Tanggal Rilis';
            labelWaktu.className = 'text-sm font-bold text-gray-700';
            descWaktu.textContent = 'Kosongkan jika tidak ingin mengubah.';
    ...
    // For edit form without id form-berita, also handle
    document.querySelector('form').addEventListener('submit', function(e){
        if (!document.getElementById('konten')) return;
