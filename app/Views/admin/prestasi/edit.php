...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h2 class="text-2xl font-bold text-gray-800">Edit Data Prestasi</h2>
        <p class="text-sm text-gray-500">Perbarui informasi penghargaan yang sudah ada.</p>
    </div>
    ...
    <form action="<?= base_url('admin/prestasi/update/' . $prestasi['id']) ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <?php $kategoriTerpilih = old('kategori_prestasi', $prestasi['kategori_prestasi']); ?>
                ...
                <div id="form-siswa" class="hidden p-4 bg-blue-50 border border-blue-100 rounded-xl space-y-4">
                    <div>
                        ...
                        <input type="text" name="nama_pemenang" value="<?= old('nama_pemenang', $prestasi['nama_pemenang']) ?>" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#00A859] transition-colors">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Kelas</label>
                        <input type="text" name="kelas" value="<?= old('kelas', $prestasi['kelas']) ?>" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#00A859] transition-colors">
                    </div>
                </div>
                ...
                <div id="form-guru" class="hidden p-4 bg-purple-50 border border-purple-100 rounded-xl space-y-4">
                    <div>
                        ...
                        <input type="text" name="nama_guru" value="<?= old('nama_guru', $prestasi['nama_guru']) ?>" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#00A859] transition-colors">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Judul Tampilan <span class="text-red-500">*</span></label>
                    <input type="text" name="judul" value="<?= old('judul', $prestasi['judul']) ?>" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white focus:border-[#00A859] transition-colors" required>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nama Lomba / Event <span class="text-red-500">*</span></label>
                    <input type="text" name="nama_lomba" value="<?= old('nama_lomba', $prestasi['nama_lomba']) ?>" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white focus:border-[#00A859] transition-colors" required>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Peringkat / Juara <span class="text-red-500">*</span></label>
                        <input type="text" name="juara" value="<?= old('juara', $prestasi['juara']) ?>" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white focus:border-[#00A859] transition-colors" required>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Tahun <span class="text-red-500">*</span></label>
                        <input type="number" name="tahun_perolehan" value="<?= old('tahun_perolehan', $prestasi['tahun_perolehan']) ?>" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white focus:border-[#00A859] transition-colors" required>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:bg-gray-50 transition-colors cursor-pointer" id="upload-area" onclick="document.getElementById('gambar').click()">
                        ...
                        <p class="text-sm font-bold text-gray-600 mb-1">Klik untuk mengganti gambar</p>
                        <p class="text-xs text-gray-400">Kosongkan jika tidak ingin mengganti gambar saat ini.</p>
                        ...
                        <div id="preview-container" class="mt-4 relative <?= empty($prestasi['gambar']) ? 'hidden' : '' ?>">
                            <img id="image-preview" src="<?= !empty($prestasi['gambar']) ? base_url('uploads/prestasi/' . $prestasi['gambar']) : '' ?>" class="mx-auto rounded-lg max-h-48 object-cover shadow-sm">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" rows="5" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white focus:border-[#00A859] transition-colors"><?= old('konten', $prestasi['konten']) ?></textarea>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Nama Trofi/Penghargaan (Opsional)</label>
                    <input type="text" name="nama_penghargaan" value="<?= old('nama_penghargaan', $prestasi['nama_penghargaan']) ?>" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:bg-white focus:border-[#00A859] transition-colors">
                </div>
            </div>
        ...
        <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-3">
            <button type="submit" class="px-6 py-2.5 bg-[#00A859] hover:bg-[#0B4A2D] text-white font-bold rounded-xl transition-colors shadow-sm">
                <i class="fa-solid fa-save mr-2"></i> Update Prestasi
            </button>
        </div>
...
<script>
    ...
    // Jalankan otomatis saat halaman diload untuk menyesuaikan data lama
